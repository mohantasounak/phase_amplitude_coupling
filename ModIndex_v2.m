% [MI,MeanAmp2]=ModIndex_v2(Phase, Amp, position)
%
...
% MeanAmp2 = MeanAmp(find(isnan(MeanAmp) == 0))
% nbin2 = length(MeanAmp2)
MI=(log(nbin)-(-sum((MeanAmp/sum(MeanAmp)).*log((MeanAmp/sum(MeanAmp))))))/log(nbin);
% MI=(log(nbin2)-(-sum((MeanAmp2/sum(MeanAmp2)).*log((MeanAmp2/sum(MeanAmp2))))))/log(nbin2);
