...
function [MI,MeanAmp] = ModIndex_v1(data_channel_avg_A1,Pf1,Pf2,Af1,Af2,position)

% the eegfilt routine employed below is obtained from the EEGLAB toolbox 
% (Delorme and Makeig J Neurosci Methods 2004)

    [freq_A1] = prepoc_hilbert_PAC(data_channel_avg_A1, [Pf1 Pf2], 'angle' );
%     if data was not averaged over trials then uncomment next line
%     Phase = squeeze(circ_mean(freq_A1.powspctrm  , [], 1))';
    Phase = freq_A1.powspctrm;
...
% Amp=abs(hilbert(AmpFreq)); % getting the amplitude envelope
    ...
    [ freq_A2] = prepoc_hilbert_PAC(data_channel_avg_A1, [Af1 Af2], 'abs' );
%     if data was not averaged over trials then uncomment next line
%     Amp= squeeze(mean(freq_A2.powspctrm  , 1))';
    Amp  = freq_A2.powspctrm;
